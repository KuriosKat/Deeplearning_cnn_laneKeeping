...
# 이미지 데이터셋 경로 설정
data_path = "untrained_dataset_mission2_0815_2145_YH_augmented2"  # data 폴더 안에 각 클래스 폴더 (0, 1, 2)가 있어야 함
...
for epoch in range(num_epochs):
    ...
    if (epoch + 1) % save_interval == 0:
        model_filename = f"lane_classifier_model_class6_epoch{epoch+1}_0815.pth"
        torch.save(model.state_dict(), model_filename)
        print(f"Saved model weights at epoch {epoch+1}")

# 모델 저장
torch.save(model.state_dict(), "lane_classifier_model_class6_epoch70_0815.pth")
